 ...
 * grading a real run. What it does check is everything between the two
 * buttons and the disk: which route carries speech and which never does, that
 * a screenshot goes out on every solve, that the transcript reaching the model
 * is the speech that was actually captured, and that each problem leaves the
 * right pair of log lines behind -- including the bail on a voice-only
 * problem, which is the one outcome a quiz like this exists to pin down.
 */
...
/**
 * Puts one problem on screen, says its lines, presses its button, and lets the
 * fake provider answer with the fixture's own scripted answer.
 *
 ...
 * the last test in this file is about it.
 */
async function ask(app: E2EApp, entry: MockQuizProblem, callIndex: number) {
  ...
  const response =
    solveRoute(entry) === '/solve' ? await app.solve() : await app.solveWithTranscript();
  assert.equal(response.status, 202, `${entry.id} was accepted`);

  const call = await app.provider.waitForCall(callIndex);
  call.answer(entry.scriptedAnswer);
  return call;
}
...
describe('e2e: the mock quiz, voice-only problems', () => {
  it('carries the spoken question to the model, and still records the bail the screen decides', async (t) => {
    const app = await bootQuiz(t);
    ...
    assert.ok(call.image.bytes.length > 0, 'and the screenshot goes with it -- speech never replaces the screen');
    ...
    assert.equal(usage?.bail, true, 'the attempt is still accounted for, marked as a bail');
    ...
    // reaching `on`, the target the quiz moved to, every finalized line as it
    // was said, and then the solve itself.
  ...
  it('the second voice-only problem behaves the same way from a fresh recording session', async (t) => {
...
describe('e2e: the mock quiz, voice problems about what is on screen', () => {
  ...
  it('only sends what was said before the button, not what is said after it', async (t) => {
    ...
    second.answer(entry.scriptedAnswer);
    await app.waitForAnswerLines(2);
  });
});

describe('e2e: the whole mock quiz, start to finish', () => {
  it('walks every problem in order and leaves exactly the logs each kind calls for', async (t) => {
    const app = await bootQuiz(t);
    await app.startRecording();

    for (const [index, entry] of quiz.problems.entries()) {
      const call = await ask(app, entry, index + 1);
      const carriedSpeech = 'transcript' in (call.options ?? {});
      assert.equal(
        carriedSpeech,
        entry.kind !== 'screen',
        `${entry.id}: a "${entry.kind}" problem is asked ${entry.kind === 'screen' ? 'without' : 'with'} speech`,
      );
    }
    ...
    assert.deepEqual(
      ...
      'the bails are exactly the voice-only problems',
    );
    assert.deepEqual(
      usage.map((line) => line.withTranscript === true),
      quiz.problems.map((entry) => entry.kind !== 'screen'),
    );
    assert.deepEqual(
      answers.map((answer) => answer.title),
    ...
    // Nothing said during the quiz was ever answered *instead of* the screen:
    // every bail names the v1 marker verbatim.
    for (const entry of quiz.problems.filter((candidate) => candidate.expected.outcome === 'bail')) {
      assert.equal(entry.expected.title, BAIL_TITLE);
    }
  });

  it('a voice problem asked with recording off still solves, from the screen alone', async (t) => {
