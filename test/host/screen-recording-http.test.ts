...
import type { OpenRecorder } from '../../src/host/screen-recording/types.ts';
import { tempStateRoot } from '../helpers/temp-state-root.ts';

/**
 * #47's HTTP surface: the recorder's control and playback endpoints, plus the
 * `recording` SSE frame.
 *
...
async function startTestServer(
  ...
  const { routes } = createHostRoutes({
    configStore,
    screenRecordingCoordinator: options.withCoordinator === false ? undefined : coordinator,
    screenRecordingLog,
    recordingsDir,
    logger: silentLogger,
  });
...
describe('recording HTTP surface', () => {
  describe('POST /screen-recording/start', () => {
    it('starts recording and answers with the live snapshot', async (t) => {
      const h = await startTestServer(t);

      const response = await fetch(`${h.server.url}/screen-recording/start`, { method: 'POST' });
  ...
  it('GET /screen-recording reports the state a freshly-loaded client needs before any SSE frame', async (t) => {
    const h = await startTestServer(t);

    const before = (await (await fetch(`${h.server.url}/screen-recording`)).json()) as { state: string };
    assert.equal(before.state, 'off');

    await fetch(`${h.server.url}/screen-recording/start`, { method: 'POST' });

    const after = (await (await fetch(`${h.server.url}/screen-recording`)).json()) as { state: string };
    assert.equal(after.state, 'recording');
  });

  it('POST /screen-recording/stop returns to off', async (t) => {
    const h = await startTestServer(t);
    await fetch(`${h.server.url}/screen-recording/start`, { method: 'POST' });

    const response = await fetch(`${h.server.url}/screen-recording/stop`, { method: 'POST' });

    assert.equal(response.status, 200);
    assert.equal(((await response.json()) as { state: string }).state, 'off');
  });

  describe('GET /screen-recordings', () => {
    it('is empty before anything has been recorded', async (t) => {
      const h = await startTestServer(t);

      const response = await fetch(`${h.server.url}/screen-recordings`);
    ...
    it('lists a segment as soon as it opens, before it has been closed out', async (t) => {
      ...
      const segments = (await (await fetch(`${h.server.url}/screen-recordings`)).json()) as ScreenRecordingSegment[];
  ...
  describe('GET /screen-recordings/file', () => {
    ...
    it('serves the whole file, advertising range support', async (t) => {
      ...
      const response = await fetch(`${h.server.url}/screen-recordings/file?id=abc`);
    ...
    it('404s an id that is not in the index', async (t) => {
      ...
      const response = await fetch(`${h.server.url}/screen-recordings/file?id=nope`);
    ...
    it('404s an indexed segment whose file has vanished from disk', async (t) => {
      ...
      const response = await fetch(`${h.server.url}/screen-recordings/file?id=ghost`);
    ...
    it('refuses a traversal attempt rather than reading outside the recordings directory', async (t) => {
      ...
      const response = await fetch(
        `${h.server.url}/screen-recordings/file?id=${encodeURIComponent('../../config')}`,
      );
    ...
    it('400s a request with no id at all', async (t) => {
      ...
      assert.equal((await fetch(`${h.server.url}/screen-recordings/file`)).status, 400);
    });
  });
  ...
  describe('recording settings', () => {
    it('round-trips a partial patch, leaving the untouched fields alone', async (t) => {
      ...
      const response = await fetch(`${h.server.url}/screen-recording/settings`, {
        method: 'POST',
      ...
      assert.equal(h.configStore.get().screenRecording.enabled, true, 'and it reached the store');
    });
    ...
    it('rejects a field of the wrong type with 400, rather than silently dropping it', async (t) => {
      ...
      assert.equal(response.status, 400);
      assert.equal(h.configStore.get().screenRecording.enabled, false, 'and nothing changed');
    });

    it('persists across a reload of the store, which is what makes automatic recording automatic', async (t) => {
      const h = await startTestServer(t);
      await fetch(`${h.server.url}/screen-recording/settings`, {
        method: 'POST',
      ...
      assert.equal(reloaded.get().screenRecording.enabled, true);
      assert.equal(reloaded.get().screenRecording.retentionDays, 3);
    });
  });
  ...
  describe('the recording SSE frame', () => {
    it('broadcasts a state change to a connected client', async (t) => {
      ...
      assert.equal(frame?.type, 'screen-recording');
      assert.ok(
