...
import { foldScreenRecordingLog } from '../../src/host/logs/screen-recording-log.ts';
import type { ScreenRecordingLogEntry } from '../../src/host/logs/types.ts';

describe('foldScreenRecordingLog', () => {
  it('returns empty for empty input', () => {
    assert.deepEqual(foldScreenRecordingLog([]), []);
  });

  it('an opened with no closed folds to a segment with endedAt: null, bytes: 0, durationMs: null', () => {
    const entries: ScreenRecordingLogEntry[] = [
      { type: 'opened', id: 'a', startedAt: '2026-08-01T00:00:00.000Z', mimeType: 'video/webm', target: null },
    ];
    const result = foldScreenRecordingLog(entries);
  ...
  it('a pruned entry removes the segment entirely from the output', () => {
    ...
    assert.deepEqual(foldScreenRecordingLog(entries), []);
  });
  ...
  it('a closed for an id that was never opened is ignored rather than throwing', () => {
    ...
    assert.doesNotThrow(() => foldScreenRecordingLog(entries));
    assert.deepEqual(foldScreenRecordingLog(entries), []);
  });

  it('a pruned for an id that was never opened is ignored rather than throwing', () => {
    const entries: ScreenRecordingLogEntry[] = [{ type: 'pruned', id: 'ghost', prunedAt: '2026-08-01T00:05:00.000Z' }];
    assert.doesNotThrow(() => foldScreenRecordingLog(entries));
    assert.deepEqual(foldScreenRecordingLog(entries), []);
  });
